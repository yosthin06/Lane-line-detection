...
def pipeline(img_name):
    ...
    img_colour = input_image(img_name)
    
    #2.- Convert from BGR to RGB then from RGB to greyscale
    grey = greyscale_img(img_colour)

    # 3.- Apply Gaussian smoothing
    # ---filtrado espacial de la imagen---
    kernel_size = (9, 9)
    blur_grey = smoothed_img(grey, kernel_size)
    
    #Apply Canny edge detector
    low_threshold = 70
    high_threshold = 100
    edges = canny_img(blur_grey, low_threshold, high_threshold)
    ...
    hough_lines = hough(img_colour, roi_image, rho, theta, threshold, min_line_len, max_line_gap)
    ...
    left_line_x, left_line_y, right_line_x, right_line_y = left_and_right_lines(hough_lines, img_colour)
    
    defined_lane_lines = lane_lines(left_line_x, left_line_y, right_line_x, right_line_y, img_colour)      
